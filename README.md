...
| Tool | Description |
|------|-------------|
| `adr_add` | Record a new decision in one call (title, context, decision, optional consequences and considered options -- each a structured list of positioned entries with its own classification, statement+type / name+rationale+outcome -- plus the requirement codes it addresses, the bounded-context codes it affects and the codes of the decisions it is related to; each referenced resource must already exist). Starts out PROPOSED. Title, context, decision and every consequence/considered-option text are natively multilingual -- an optional `language` argument tags the literals being written (BCP-47, e.g. `"en"`); omitted, it falls back to the project's configured default language, and rejects the call if the project has none either |
...
| `adr_delete` | Delete a decision and every triple it carries -- the whole resource, not a field correction. Only a PROPOSED decision can be deleted: this undoes a record created by mistake. From ACCEPTED on the record stays: an ACCEPTED decision is superseded or marked DEPRECATED, while a REJECTED, DEPRECATED or SUPERSEDED one simply stays -- the refusal names the path that fits the status. REJECTED is no way out either -- "considered and turned down" is itself a decision. Rejected while another decision names it as its own successor or points at it via `relatedTo`; the refusal names those decisions. The freed code is not handed out again |
...
| `orphan_check` | Orphaned artefacts: requirements without a realizing use case, glossary terms never referenced (neither used by a requirement or a use case, nor playing an actor role in a use case, nor a bounded context's ubiquitous language, nor another term's broader term), text mentions of a term missing its `usesTerm`/`primaryActor`/`supportingActor`/`ubiquitousLanguageTerm`/`broader` edge -- scanning a requirement's/use case's/bounded context's own text (a use case's goal, scope, trigger, precondition, postcondition and every step/extension text count as its text) and a term's own definition mentioning another term it does not name as its broader term -- and constraints no requirement or use case is bound by via `constrainedBy` |
| `impact_analysis` | Transitive "who references this" closure for a resource handle -- what is affected if X changes, following the requirement/use-case/glossary/bounded-context edges plus an ADR's `addressesRequirement`/`affectsContext` backwards and its `supersededBy` forwards (see sample below) |
...
| `arknet-adr` | Sixth hexagonal BC: architecture decision records -- context, decision, and consequences/considered options as their own positioned resources (each with a classification: consequence type POSITIVE/NEGATIVE/NEUTRAL, considered-option outcome CHOSEN/REJECTED), plus the edges to the requirement a decision addresses, the bounded context it affects, and its successor (`arkarch:supersededBy`, written on the superseded decision together with its SUPERSEDED status) (core + kognio-rdf out-adapter + MCP/Spring AI in-adapter). Store-backed and numbered independently of the hand-written markdown decision records under `docs/adr/` |
