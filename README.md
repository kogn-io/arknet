...
| `bc_link_term` | Link a bounded context to a glossary term (`arkddd:ubiquitousLanguageTerm`; the term must exist). Linking the same term twice is a no-op. To remove a link (or replace the whole set), use `bc_update`'s `terms` instead |
