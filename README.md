...
| `store_check` | Checks the stored model against what the project declares about itself and reports what is decidable -- read-only, changing nothing and refusing nothing. Pick rules with `checks`, or omit it to run all of them; today there is exactly one. `LANGUAGE` lists every field that carries at least one language-tagged value but not one for each language the project maintains (`project_update languages`), as a table of resource, type, field and missing tags -- a positioned child resource with no code of its own is named by its owner's code plus its position (`FR-1 acceptanceCriterion#2`). A project that declares no maintained language set gets an explicit "not checked" instead of a clean result: with no declared set there is no target state to be incomplete against. What it does not see travels in its own output: a field carrying no language-tagged value at all is indistinguishable from one that is simply not multilingual, and it judges presence per language only, never whether one language's text is still a current translation of another's |
