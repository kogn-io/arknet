...
| `orphan_check` | Orphaned artefacts: requirements without a realizing use case, glossary terms never referenced (usage, bounded-context language, or being another term's broader term), text mentions of a term missing its `usesTerm`/`ubiquitousLanguageTerm`/`broader` edge -- including a term's own definition mentioning another term it does not name as its broader term -- and constraints no requirement is bound by via `constrainedBy` |
