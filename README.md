...
| `req_update` | Correct a requirement's title, description, rationale and/or MoSCoW priority (each optional, unchanged if omitted -- and this is also how a requirement created without a rationale records one afterwards); append new acceptance criteria and/or correct an existing one's text by position -- position is purely technical, so mid-list insert/delete/reorder is not supported. `language` scopes a non-omitted title/description/rationale/touched-criterion write to that one language's literal (falling back to the project's default language if omitted, rejecting the call if neither is set), leaving other language variants untouched -- except a stale untagged one, swept away once the resolved tag equals the project's default |
...
| `uc_update` | Correct a use case's title, goal, scope, trigger, precondition, postcondition, role references and/or extensions (each optional, unchanged if omitted), and/or individual existing steps' text and/or FR-realises references by position (a listed step's realises set is replaced wholesale, empty to clear it). A given `primaryRole` replaces the current one and cannot be cleared (a use case always has exactly one); a given `supportingRoles` array replaces the current list wholesale, empty to clear it -- does not touch the step list's structure. `language` scopes a non-omitted title/goal/scope/trigger/precondition/postcondition/step-text/extension-text write to that one field's/position's literal (falling back to the project's default language if omitted, rejecting the call if neither is set), leaving other language variants untouched -- except a stale untagged one, swept away once the resolved tag equals the project's default |
