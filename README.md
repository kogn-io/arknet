...
| `adr_get` | Fetch a single decision with its full text, both directions of the supersedes relation, and every decision it is related to (e.g. ADR-1) |
...
| `adr_set_status` | Change a decision's lifecycle status: PROPOSED -> ACCEPTED, PROPOSED -> REJECTED, or ACCEPTED -> DEPRECATED. REJECTED means the option was considered and turned down -- a record worth keeping, not a way to get rid of one recorded by mistake |
| `adr_supersede` | Record that one decision replaces an older one (`arkarch:supersedes`). Only the forward edge is stored -- the superseded decision reports it as "superseded by" from a reverse read, not from a second triple. `arkarch:relatedTo`, the cross-reference between equals, has no tool of its own: it is set with `adr_add`/`adr_update`, stored forward-only in the same way, and read back as one merged list because the relation is symmetric |
| `adr_delete` | Delete a decision and every triple it carries -- the whole resource, not a field correction. Only a PROPOSED decision can be deleted: this undoes a record created by mistake. From ACCEPTED on the record stays (supersede it, or mark it DEPRECATED), and REJECTED is no way out either -- "considered and turned down" is itself a decision. Rejected while another decision still points at it via `supersedes`/`relatedTo`; the refusal names those decisions. The freed code is not handed out again |
