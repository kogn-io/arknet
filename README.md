...
| Tool | Description |
|------|-------------|
| `adr_add` | Record a new decision in one call (title, context, decision, optional consequences and considered options -- each a structured list of positioned entries with its own classification, statement+type / name+rationale+outcome -- and decision date, plus the requirement codes it addresses, the bounded-context codes it affects and the codes of the decisions it is related to; each referenced resource must already exist). Starts out PROPOSED. Title, context, decision and every consequence/considered-option text are natively multilingual -- an optional `language` argument tags the literals being written (BCP-47, e.g. `"en"`); omitted, it falls back to the project's configured default language, and rejects the call if the project has none either |
...
| `adr_set_status` | Change a decision's lifecycle status: PROPOSED -> ACCEPTED, PROPOSED -> REJECTED, or ACCEPTED -> DEPRECATED. REJECTED means the option was considered and turned down -- a record worth keeping, not a way to get rid of one recorded by mistake. SUPERSEDED is not settable here -- it needs a successor, which only `adr_supersede` can name |
