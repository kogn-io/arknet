...
| `role_update` | Correct a role's name and/or description (each optional, unchanged if omitted), state either in a further language, and/or replace who fills it. `filledBy` uses its own tri-state: a list replaces the occupancy wholesale, an empty list clears it, omitted leaves it untouched. Code stays as created |
