...
| `orphan_check` | Orphaned artefacts: requirements without a realizing use case, glossary terms never referenced (neither used by a requirement, a use case or an architecture decision (`arkarch:usesTerm`, kogn-io/arknet#393), nor playing an actor role in a use case, nor a bounded context's ubiquitous language, nor another term's broader term), text mentions of a term missing its `usesTerm`/`primaryActor`/`supportingActor`/`ubiquitousLanguageTerm`/`broader` edge -- scanning a requirement's/use case's/bounded context's own text (a use case's goal, scope, trigger, precondition, postcondition and every step/extension text count as its text) and a term's own definition mentioning another term it does not name as its broader term -- and constraints no requirement or use case is bound by via `constrainedBy` |
