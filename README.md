...
| `req_link_term` | Link a requirement to a glossary term (`arkreq:usesTerm`; the term must exist). Linking the same term twice is a no-op. To remove a link (or replace the whole set), use `req_update`'s `usesTermCodes` instead |
