...
| `adr_delete` | Delete a decision and every triple it carries -- the whole resource, not a field correction. Only a PROPOSED decision can be deleted: this undoes a record created by mistake. From ACCEPTED on the record stays: an ACCEPTED decision is superseded or marked DEPRECATED, while a REJECTED, DEPRECATED or SUPERSEDED one simply stays -- the refusal names the path that fits the status (a SUPERSEDED decision that got there by mistake is corrected with `adr_unsupersede`, not deleted). REJECTED is no way out either -- "considered and turned down" is itself a decision. Rejected while another decision names it as its own successor (removable with `adr_unsupersede` on that decision) or points at it via `relatedTo`; the refusal names those decisions. The freed code is not handed out again |
