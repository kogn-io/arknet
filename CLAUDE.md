...
Abgelehnt wird ausserdem, solange ein anderer Record ueber `supersedes` oder `relatedTo` darauf zeigt; der Code eines geloeschten Records bleibt vergeben (er ueberlebt als `dcterms:identifier` an der getombstoneten Revision), damit `ADR-7` nie zwei Entscheidungen benennt.
