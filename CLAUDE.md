...
- **arknet-project**: einzige Komponente des Bounded Context Projekt-Registry (BC-5), kein Vokabularmodul (ADR-57), aber ein eigener Kontext-Parent -- die Projekt-Registry (`project_*`), bildet den Anker eines Aufrufs auf das Projekt ab; einziger nicht projekt-scoped BC (System-Dataset); fuehrt Standardsprache und Sprachsatz des Projekts. `arknet-project-registry/arknet-project/CLAUDE.md`
...
- **arknet-actor**: einzige Komponente des Bounded Context Akteur (BC-4) -- Actor-Lifecycle (`actor_*`, `ACTOR-N`) und Role als zweiter Ressourcentyp (`role_*`, `ROLE-N`, `filledBy`). `arknet-actor/CLAUDE.md`
- **arknet-model-analysis**: Kontext-Parent (packaging pom, kein Code) des Bounded Context Modellanalyse (BC-6) mit seiner einen, rein lesenden Komponente (`-core`, `-adapter-kogniordf`, `-adapter-mcp`, Package `de.hauschel.arknet.analysis`) -- die fuenf kontextuebergreifenden Auswertungen (`impact_analysis`, `trace_matrix`, `orphan_check`, `role_usecase_matrix`, `term_cooccurrence`) und `store_check`; besitzt keine Ressource und liest jeden anderen Kontext ueber dessen Published Language. `arknet-model-analysis/CLAUDE.md`
