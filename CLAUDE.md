...
- **arknet-adr**: sechste hexagonale BC -- Architecture-Decision-Record-Lifecycle (`adr_add`/`adr_list`/`adr_get`/`adr_update`/`adr_set_status`/`adr_supersede`), `addressesRequirement`-Kante zu den Requirements, `affectsContext`-Kante zu den Bounded Contexts, zwei selbstbezuegliche Kanten (`supersedes` als Lifecycle-Akt mit eigenem Tool, `relatedTo` als gleichrangiger Querverweis, ueber `adr_add`/`adr_update` setzbar) -- von beiden wird nur die Vorwaertsrichtung als Tripel geschrieben, opake Identitaet.
