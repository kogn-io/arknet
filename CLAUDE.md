...
- Repository: Code und Pull Requests leben auf GitHub (`github.com/kogn-io/arknet`, Apache-2.0).
  ...
  `deferred`), offene Fragen ueber GitHub Discussions. Milestones sind Release-Schnitte, keine
  Themenkoerbe, und heissen `X.Y.Z Thema` (Version vorne, Thema hinten, z.B.
  `0.7.0 Stabil+Sprache`). Jedes offene Issue traegt einen; ohne Themenbindung gehoert es in
  einen spaeteren, nicht in gar keinen. Milestone-frei bleiben allein ein Epic, das mehrere
  Milestones spannt oder vor ihnen laeuft, und `vision`-Issues, die Scope statt Release
  tragen. Siehe README "Repository"-Abschnitt.
...
- **arknet-product-requirements**: Kontext-Parent (packaging pom, kein Code) des Bounded Context Produkt & Anforderungen (BC-1); darunter das Vokabularmodul arknet-product-requirements-shared und die zwei Komponenten arknet-requirements und arknet-use-cases. `arknet-product-requirements/CLAUDE.md`
- **arknet-product-requirements-shared**: Vokabularmodul von Produkt & Anforderungen -- `RequirementCode`, `ConstraintCode`, `TermRef`, Package `de.hauschel.arknet.pr.shared`. `arknet-product-requirements/arknet-product-requirements-shared/CLAUDE.md`
- **arknet-requirements**: Komponente von Produkt & Anforderungen -- Requirement-Lifecycle (`req_*`) plus Constraint als zweiter Ressourcentyp (`constraint_*`), `usesTerm`-Kante ins Glossar. `arknet-product-requirements/arknet-requirements/CLAUDE.md`
- **arknet-use-cases**: Komponente von Produkt & Anforderungen -- Cockburn-Use-Cases (`uc_*`), Rollen per `ROLE-N`-Code aus dem Actor-Register, Kanten zu Glossar und Constraints. `arknet-product-requirements/arknet-use-cases/CLAUDE.md`
- **arknet-domain-modelling**: Kontext-Parent (packaging pom, kein Code) des Bounded Context Domain Modelling (BC-2); darunter das Vokabularmodul arknet-domain-modelling-shared und die zwei Komponenten arknet-ubiquitous-language und arknet-bounded-context. `arknet-domain-modelling/CLAUDE.md`
- **arknet-domain-modelling-shared**: Vokabularmodul von Domain Modelling -- `TermCode`, Package `de.hauschel.arknet.dm.shared`. `arknet-domain-modelling/arknet-domain-modelling-shared/CLAUDE.md`
- **arknet-ubiquitous-language**: Komponente von Domain Modelling -- SKOS-Glossar (`term_*`). `arknet-domain-modelling/arknet-ubiquitous-language/CLAUDE.md`
- **arknet-bounded-context**: Komponente von Domain Modelling -- BoundedContext-Lifecycle (`bc_*`), ContextRelationship als eigene Ressource. `arknet-domain-modelling/arknet-bounded-context/CLAUDE.md`
- **arknet-architecture-decisions**: Kontext-Parent (packaging pom, kein Code) des Bounded Context Architektur & Entscheidungen (BC-3); kein Vokabularmodul (ADR-57), darunter die eine Komponente arknet-adr. `arknet-architecture-decisions/CLAUDE.md`
- **arknet-adr**: einzige Komponente des Bounded Context Architektur & Entscheidungen (BC-3) -- Architecture-Decision-Record-Lifecycle (`adr_*`, `adr_check`), Kanten zu Requirements, Bounded Contexts und Glossar, `supersededBy`/`relatedTo` selbstbezueglich; arknets eigene ADRs sind Records dieses Hexagons. `arknet-architecture-decisions/arknet-adr/CLAUDE.md`
- **arknet-actor**: Kontext-Parent (packaging pom, kein Code) des Bounded Context Akteur (BC-4); kein Vokabularmodul (ADR-57), darunter die eine Komponente arknet-actor-register. `arknet-actor/CLAUDE.md`
- **arknet-actor-register**: einzige Komponente des Bounded Context Akteur (BC-4) -- Actor-Lifecycle (`actor_*`, `ACTOR-N`) und Role als zweiter Ressourcentyp (`role_*`, `ROLE-N`, `filledBy`). `arknet-actor/arknet-actor-register/CLAUDE.md`
- **arknet-project-registry**: Kontext-Parent (packaging pom, kein Code) des Bounded Context Projekt-Registry (BC-5); kein Vokabularmodul (ADR-57), darunter die eine Komponente arknet-project. `arknet-project-registry/CLAUDE.md`
- **arknet-project**: einzige Komponente des Bounded Context Projekt-Registry (BC-5) -- die Projekt-Registry (`project_*`), bildet den Anker eines Aufrufs auf das Projekt ab; einziger nicht projekt-scoped BC (System-Dataset); fuehrt Standardsprache und Sprachsatz des Projekts. `arknet-project-registry/arknet-project/CLAUDE.md`
- **arknet-model-analysis**: Kontext-Parent (packaging pom, kein Code) des Bounded Context Modellanalyse (BC-6); kein Vokabularmodul (ADR-57), darunter die eine Komponente arknet-model-evaluation. `arknet-model-analysis/CLAUDE.md`
- **arknet-model-evaluation**: einzige, rein lesende Komponente des Bounded Context Modellanalyse (BC-6) (`-core`, `-adapter-kogniordf`, `-adapter-mcp`, Package `de.hauschel.arknet.analysis`) -- die fuenf kontextuebergreifenden Auswertungen (`impact_analysis`, `trace_matrix`, `orphan_check`, `role_usecase_matrix`, `term_cooccurrence`) und `store_check`; besitzt keine Ressource und liest jeden anderen Kontext ueber dessen Published Language. `arknet-model-analysis/arknet-model-evaluation/CLAUDE.md`
...
- **arknet-persistence-support**: technischer Support der kognio-rdf-Adapter -- SHACL-Write-Gate, WriteFunnel (PROV-O-Revision + Head-Pointer je Write), SparqlTerms, die `Ark*Vocabulary`-Konstanten sowie der typunabhaengige Store-Lesepfad (`StoreReader`, das Lesemodell `Triple`/`RdfNode`/`StoreResource`/`StoreSnapshot`, `Prefixes`, `HandleResolver`, `ResourceRenderer`, `Revision`), den die Composition Root und der Out-Adapter der Modellanalyse teilen. `arknet-support/arknet-persistence-support/CLAUDE.md`
...
- **arknet-persistence-test-support**: Test-Support derselben Out-Adapter -- die `DatasetLifecycle`/`DatasetTx`-Dekoratoren, mit denen die `*RealStoreConcurrencyTest` eine gewaehlte Verschraenkung zweier Schreiber festnageln (`Guarded*` plus die Pausier-Dekoratoren, die mehr als ein BC braucht); main-Scope-Artefakt, im test-Scope gezogen. `arknet-support/arknet-persistence-test-support/CLAUDE.md`
