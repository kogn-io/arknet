...
Ein Use Case loest `primaryRole`/`supportingRole` gegen `ROLE-N`-Codes dieses Rollen-Registers auf (`RoleLookup`/`ResolveRoles` in `arknet-use-cases`), nicht gegen den Namen -- ein Rollenname ist mehrsprachig und damit als Referenzschluessel mehrdeutig; bis ADR-37/kogn-io/arknet#405 (Teil C) loeste `arknet-use-cases` stattdessen gegen Actor-Namen auf (`ActorLookup`/`ResolveActors`, seit Issue #336).
