...
Was hier liegt, nennt kein `*-core` und kein Vokabularmodul; die Aufrufer sind ausschliesslich die sieben `*-adapter-mcp`-Module und `arknet-app`.
...
Die Implementierung `RegisteredAnchorProjectResolver` in `arknet-app` adaptiert dafuer den `ResolveProject`-In-Port des `arknet-project`-BC; die Modell-BCs sehen nur diesen neutralen Port und haengen nie an jenem BC.
...
Er liegt hier und wird im Composition Root ueber den generischen Store-Lesepfad implementiert (`StoreFieldLanguageLookup` in `arknet-app`), dieselbe Richtung wie bei `ProjectResolver`: ein Port neben den Adaptern, eine Implementierung im Composition Root, konsumiert von jedem BC-In-Adapter, ohne eine einzige neue Modulkante.
...
Sie sind bewusst kurz und sagen nur, *wann* ein Aufrufer den Parameter setzt; die ausfuehrliche Erklaerung (`projectAnchor` als Override des `X-Arknet-Project-Anchor`-Headers, der Projekt-Default als Rueckfall von `language` und `displayLocale`) steht einmal in `arknet-app`s `spring.ai.mcp.server.instructions`, die volle `displayLocale`-Fallback-Kette am `DisplayLocale#select` des Kernels.
