 ...
 * and/or its two cross-context reference lists - leaving {@code status} and {@code supersedes} to
 * their own ports ({@code adr_set_status}, {@code adr_supersede}).
 *
 ...
 * shape: a flat list of eight mostly-{@code null} arguments, five of them adjacent
 * {@link String}s, puts every caller one silent transposition away from writing the wrong field.</p>
 *
 ...
 *
 * <p><strong>The two reference lists are the deliberate exception and stay correctable in every
 * status.</strong> Adding an {@code addressesRequirement} or {@code affectsContext} edge later
 * completes a reference that could not be written when the decision was recorded - the requirement
 * or bounded context did not exist yet - rather than changing what was decided. The precedent is
 * {@code adr_supersede}, which already writes into an accepted decision's {@code supersedes}.</p>
 *
 * <p><strong>Tri-state reference lists.</strong> For both {@code addressesRequirementCodes} and
 * {@code affectsContextCodes}: {@code null} leaves the existing edges untouched, an empty list is
 * the explicit, unambiguous signal to remove every edge of that relation, and a non-empty list
 * replaces the relation wholesale. This is the one place where {@code null} and empty must not be
 * conflated - the same tri-state {@code UpdateUseCase}'s {@code supportingActors} carries.</p>
 */
        ...
        public static final class Builder {
            ...
            public AdrCorrection build() {
                return new AdrCorrection(name, context, decision, consequences, alternatives,
                        decisionDate, addressesRequirementCodes, affectsContextCodes);
            }
        }
    }
}
