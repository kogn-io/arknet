...
/**
 * Out-adapter: {@link TermLookup} backed by the kognio-rdf substrate, resolving a glossary term's
 * human-typed {@code dcterms:identifier} (e.g. {@code TERM-1}) to its opaque subject
 * {@link ResourceId} within the shared project store.
 *
 * <p><strong>Strict cross-BC term resolution (kogn-io/arknet#393).</strong> Architecture decisions
 * and glossary terms share one per-project store. This adapter looks up a code by
 ...
 * the identifier, never the {@code skos:prefLabel}, so a link survives relabelling the term. This
 * is called once, from the application service, at the moment a decision is recorded -
 * {@link KognioRdfAdrRepository} never performs this lookup itself; it just persists the
 * {@link ResourceId} it is handed. Structurally 1:1 to the sibling requirements/use-cases/
 * bounded-context adapters' own {@code KognioRdfTermLookup}, and to this hexagon's own
 * {@link KognioRdfRequirementLookup}/{@link KognioRdfBoundedContextLookup}.</p>
 *
...
public final class KognioRdfTermLookup implements TermLookup {
    ...
    // share one project dataset; resolving a term means reading across into that sibling graph.
        ...
        try (DatasetHandle handle = lifecycle.acquire(new DatasetId(projectId.value()))) {
            ...
            if (matches.isEmpty()) {
                throw new UnresolvedReferenceException("Term '" + termCode
                        ...
                        + "'. Create it first with term_add before an architecture decision uses it.");
            }
            if (matches.size() > 1) {
