...
public final class AdrMcpTools {
    ...
    @McpTool(name = "adr_set_status", description = "Change the lifecycle status of an architecture "
            ...
            + "recorded by mistake: use adr_delete for that, which removes a PROPOSED decision, or "
            + "an ACCEPTED one with no successor and nothing pointing at it, outright. Moving to "
            + "ACCEPTED or REJECTED also records the decision date - this is "
    ...
    @McpTool(name = "adr_delete", description = "Delete a recorded architecture decision and every "
            ...
            + "Deletable while PROPOSED (undoes a record created by mistake - a duplicate, a draft "
            + "that belongs elsewhere), or while ACCEPTED with no successor and nothing pointing at "
            + "it - undoing a mistaken acceptance, e.g. a record that was never really an "
            + "architecture decision. A real decision that has merely become obsolete is not "
            + "deleted this way: it is superseded with adr_supersede, or marked DEPRECATED with "
            + "adr_set_status when it has no successor. Once a decision has a real place in the "
            + "model's history - superseded, deprecated, or deliberately turned down - the record "
            + "stays as it is: neither of those two paths is open there, and the refusal names the "
            + "one that fits the status. REJECTED in particular is not a way to get rid of a "
