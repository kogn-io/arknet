 ...
 * a module {@code arknet-use-cases-core} does not (and must not) depend on; {@link TermLookup}
 * itself only promises "some runtime exception", so this fake's own signal is enough to prove
 * {@link UseCaseService#linkTerm} lets a lookup failure propagate. Mirrors the sibling
 * requirements bounded context's own {@code InMemoryTermLookup}.
 */
