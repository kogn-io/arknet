 ...
 * carries no origin directory, and which was not given an explicit {@code anchor} parameter
 * either, is a caller error - never a silent fallback to some server-side working directory. Such
 * a call would have no way to know which project it belongs to, and inventing an answer is
 * precisely the failure mode this bounded context exists to close off. {@link #add} and the
 * tools that resolve their target project from the calling context ({@link #attachAnchor},
 * {@link #rename}) all throw {@link IllegalArgumentException} with a message pointing at
 * {@code project_add} in that situation, rather than guessing.</p>
 */
public final class ProjectMcpTools {

    private static final String NO_CONTEXT_ANCHOR_MESSAGE =
            "No anchor available: the calling client's transport supplied no origin directory, so "
                    ...
                    + "fallback to a server-side working directory. Register this directory as a project "
                    + "with project_add first - it also accepts an explicit anchor parameter for clients "
                    + "that cannot supply an origin directory via their transport context.";
     ...
     */
    private static Anchor requireContextAnchor(final McpSyncRequestContext context) {
        final String origin = originDir(context);
        if (origin == null || origin.isBlank()) {
            throw new IllegalArgumentException(NO_CONTEXT_ANCHOR_MESSAGE);
        }
    ...
    public String attachAnchor(
            ...
            @McpToolParam(description = "The NEW anchor to attach to the caller's own project - not "
                    + "the calling directory itself, which is used only to look up which project this "
                    + "call already belongs to.")
            final String anchor,
            @McpToolParam(description = "Type of the 'anchor' parameter: 'path', 'url' or 'uuid'. "
                    + "Defaults to 'path'.", required = false)
            final String anchorType) {
        final Project caller = resolveProject.resolve(requireContextAnchor(context));
