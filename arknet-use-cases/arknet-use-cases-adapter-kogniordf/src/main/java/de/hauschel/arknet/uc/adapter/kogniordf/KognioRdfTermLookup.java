 ...
 *
 * <p><strong>Strict cross-BC term resolution, own implementation.</strong> Use-cases and
 ...
 * ambiguous code aborts with a didactic {@link UnresolvedReferenceException}. This is called
        ...
        try (DatasetHandle handle = lifecycle.acquire(new DatasetId(projectId.value()))) {
            ...
            if (matches.isEmpty()) {
                throw new UnresolvedReferenceException("Term '" + termCode
                        ...
                        + "'. Create it first with term_add before a use case uses it.");
            }
            if (matches.size() > 1) {
