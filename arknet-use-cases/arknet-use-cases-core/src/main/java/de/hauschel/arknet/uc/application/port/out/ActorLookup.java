 ...
 * {@code arknet-actor-core}, so it cannot look an actor up as a domain object - it
 * can only ask the shared store, through this port, which resource a name currently denotes.
 * Resolution goes via the actor's {@code arknet:name}, constrained to the four concrete actor
 * types the {@code arknet-actor} register writes ({@code arkproc:HumanActor}/
 * {@code arkproc:SystemActor}/{@code arkproc:LegalActor}/{@code arkproc:GroupActor}).</p>
 *
