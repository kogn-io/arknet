 ...
 * {@link RequirementRepository#findCurrentByCode} hands out: the {@code arkprov:head} revision IRI
 * a {@link de.hauschel.arknet.persistence.WriteFunnel} write last recorded for a requirement
 ...
 * (a {@link Requirement} does not know its own head) and exists purely to let a caller round-trip a
 * value it read back to the same port for a compare-and-set check - a port-boundary concept, not an
 * aggregate field. Wrapping the bare revision IRI closes the primitive-obsession gap a raw
 * {@code String} left open: nothing at the port signature stopped a caller from passing, say, a
 * requirement code or another string entirely where a head was expected, and the compiler could not
 * catch it.</p>
 */
